"""
Inverse design of acetals by Genetic Algorithm (GA) on discrete substituent-token space,
...
Semantics:
- Search space (current stage): base+new substituent pools (cdft lookup available)
- Hard constraints enforced BEFORE oracle calls: SA < 5 and -1 < logP < 5
- No novelty vs known sets (71w+184) is enforced, but we de-duplicate within a run
  to avoid wasting budget on repeated SMILES.

Fitness:
- point target: err = |pred_LOGk2_mean_seeds - target|, fitness = -err
- range target: err = distance(pred_LOGk2_mean_seeds, [L,U]), fitness = -err
...
Outputs:
  Per run:
    ...
    - topK.csv: per-run top-K by err (optionally diversity-aware)
    - meta.json
...
def _tournament(pop: List["Ind"], k: int, rng: random.Random) -> "Ind":
    cand = rng.sample(pop, k=min(int(k), len(pop)))
    cand.sort(key=lambda r: r.fitness, reverse=True)
    return cand[0]
...
def run_one(
    ...
    # No novelty vs known, but de-dup within run.
    ...
    # init population by random fill until pop_size evaluated
    pop: List[Ind] = []
    tries = 0
    while len(pop) < int(pop_size) and len(evaluated) < int(eval_budget) and not converged:
        batch = propose_random(max(300, pop_size))
        ...
        tries += len(batch)
        for r in res:
            err = float(calc_err(float(r.pred_LOGk2_mean_seeds)))
            ind = Ind(r=r, err=err, fitness=-err)
    ...
    while len(evaluated) < int(eval_budget) and not converged:
        ...
        while len(children) < int(pop_size) and child_attempts < max_child_attempts:
            child_attempts += 2
            p1 = _tournament(pop, tournament_k, rng).r.gene
            p2 = _tournament(pop, tournament_k, rng).r.gene
        ...
        new_pop: List[Ind] = list(elites)
        cursor = 0
        while len(new_pop) < int(pop_size) and len(evaluated) < int(eval_budget) and cursor < len(children):
            ...
            for r in res:
                ...
                best_err = min(best_err, err)
                best_curve.append({"oracle_eval_idx": len(evaluated), "best_err": float(best_err)})
                if len(new_pop) >= int(pop_size) or len(evaluated) >= int(eval_budget):
                    break
            if check_convergence():
                break

        # backfill with random if too many rejects
        backfill_rounds = 0
        while len(new_pop) < int(pop_size) and len(evaluated) < int(eval_budget) and backfill_rounds < 50 and not converged:
            backfill_rounds += 1
...
def main() -> None:
    ...
    # Auto-budget rule (same as inverse BO):
    # - If K <= 100: runs=5, evals_per_run=1000
    # - If K > 100: runs=5, evals_per_run=10*K
    ap.add_argument("--runs", default=5, type=int)
    ap.add_argument("--evals_per_run", default=1000, type=int, help="If 0, auto-set based on K.")
    ...
    # pop_size auto (must be <= eval budget)
    ...
    # Aggregate across runs -> final_unique_ranked / final_topK
    try:
        parts = []
        for p in eval_paths:
